...
1. Уровни: вручную → в один клик → по скрипту → внешний планировщик.
2. Что автоматизировать: повторяющееся, с чёткими правилами, долгое простое, подверженное ошибкам.
3. Свои команды: Markdown в `.opencode/command/` (проект) или `~/.config/opencode/command/` (глобально); JSON-конфиг.
4. Параметры: `$ARGUMENTS`, `$1`/`$2`/`$3`, `` `!command` `` для shell-вывода.
5. Запуск: `/имя_команды арг1 арг2`, автоподстановка параметров.
6. Генерация скриптов: dry-run, подробный лог, минимальная команда.
7. CLI: `opencode run` неинтерактивно, для пайплайнов.
8. MCP-серверы: внешние инструменты (БД/API/поиск), следить за контекстом.
9. `/compact`: сжатие сессии, алиас `/summarize`, `Ctrl+X C`.
10. Ловушки: шаблон в frontmatter, а не в body; конфликт со встроенной командой; `` `!cmd` `` не работает — запускать из корня проекта.
