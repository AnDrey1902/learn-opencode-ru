...
1. Принцип: автоформатирование при записи/редактировании файла по расширению.
2. Встроенные: Go (gofmt), JS/TS/CSS (prettier), JS/TS (biome), Python (ruff/uv), Rust (rustfmt), C/C++ (clang-format), Shell (shfmt).
3. Конфигурация: выключить все (`false`) или конкретные; свои форматеры с плейсхолдером `$FILE`.
4. Советы: prettier сам находит зависимости, biome/json-конфиг, env-переменные.
5. Ошибки: условия активации, конфликт prettier/biome, своя команда обязана содержать `$FILE`.
