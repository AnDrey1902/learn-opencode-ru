...
- **Plan Agent**：规划分析师，主要用于分析代码和提供建议（可写 `.opencode/plans/*.md` 保存方案）
...
::: details 🔧 内部 Agent（自动运行，无需手动调用）
OpenCode 还有 3 个隐藏的内部 Agent，自动在后台工作：
...
::: tip 💡 Explore 的三种探索深度
调用 Explore 时，AI 会根据你的任务描述自动选择探索深度。你也可以在提示词中指定：
...
AI 看到"彻底""非常全面"这类描述，会自动采用 very thorough 级别。
:::
